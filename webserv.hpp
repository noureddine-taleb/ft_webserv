...
public:
  ...
  int url_changed;
  enum SchedulableEntityTypes get_type() { return RESPONSE; }
};
...
std::vector<Location>::iterator location(HttpRequest &req,
                                         std::vector<Server>::iterator server);
int check_req_line_headers(HttpRequest &request);
void response_Http_Request(int status_code, HttpRequest &request,
                           HttpResponse &response, std::string path);
...
std::string res_content_file(int status_code, HttpRequest &request,
                             HttpResponse &response, std::string path);
...
int send_response(int fd, HttpRequest &request, HttpResponse &response,
                  int status_code, bool *close_connexion);
